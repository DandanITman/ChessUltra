...
function piecePseudoMoves(board: Board, from: Square, piece: Piece, state: RulesState): Move[] {
  switch (piece.type) {
    ...
    case 'king': {
      const moves: Move[] = board.kingMoves(from, piece.color);
      // Castling generation
      const backRank = piece.color === 'white' ? 0 : 7;
      const side = state.castling[piece.color];
      const e: Square = { file: 4, rank: backRank };
      if (from.file === e.file && from.rank === e.rank) {
        // Cannot be in check
        if (!board.isSquareAttacked(e, opposite(piece.color))) {
          // King-side
          if (side.K) {
            const f: Square = { file: 5, rank: backRank };
            const g: Square = { file: 6, rank: backRank };
            const rookSq: Square = { file: 7, rank: backRank };
          ...
          // Queen-side
          if (side.Q) {
            const d: Square = { file: 3, rank: backRank };
            const c: Square = { file: 2, rank: backRank };
            const b: Square = { file: 1, rank: backRank };
            const rookSq: Square = { file: 0, rank: backRank };
    ...
    case 'pawn': {
      const moves: Move[] = board.pawnMoves(from, piece.color);
      // En passant generation
      const dir = piece.color === 'white' ? 1 : -1;
      if (state.enPassantTarget) {
        for (const dx of [-1, 1] as const) {
          const to: Square = { file: from.file + dx, rank: from.rank + dir };
          if (to.file === state.enPassantTarget.file && to.rank === state.enPassantTarget.rank) {
            // Ensure adjacent pawn exists behind target
            const capturedSq: Square = { file: to.file, rank: to.rank - dir };
...
export function legalMoves(board: Board, state: RulesState, from: Square, color: Color): Move[] {
  ...
  for (const m of pseudo) {
    ...
    for (let r = 0; r < 8; r++) {
      for (let f = 0; f < 8; f++) {
        ...
        if (p && p.type === 'king' && p.color === color) { kingSq = { file: f, rank: r }; break; }
      }
      if (kingSq) break;
    }
    if (!kingSq) continue;
    if (!clone.isSquareAttacked(kingSq, opposite(color))) res.push(m);
  }
  return res;
}
