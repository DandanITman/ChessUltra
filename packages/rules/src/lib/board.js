export class Board {
    ...
    isSquareAttacked(target, byColor) {
        ...
        for (const [dx, dy] of knightDeltas) {
            const sq = { file: target.file + dx, rank: target.rank + dy };
            const p = this.get(sq);
        ...
        for (const dx of [-1, 1]) {
            const psq = { file: target.file - dx, rank: target.rank - pawnDir };
            const p = this.get(psq);
            if (p && p.color === byColor && p.type === 'pawn')
                return true;
        }
    ...
    rayMoves(from, color, deltas) {
        const res = [];
        for (const [dx, dy] of deltas) {
            let x = from.file + dx;
            let y = from.rank + dy;
            while (x >= 0 && x < 8 && y >= 0 && y < 8) {
                const to = { file: x, rank: y };
    ...
    knightMoves(from, color) {
        ...
        for (const [dx, dy] of jumps) {
            const to = { file: from.file + dx, rank: from.rank + dy };
            if (!this.inBounds(to))
                continue;
            const occ = this.get(to);
    ...
    pawnMoves(from, color) {
        ...
        // captures
        for (const dx of [-1, 1]) {
            const cap = { file: from.file + dx, rank: from.rank + dir };
            if (!this.inBounds(cap))
                continue;
            const occ = this.get(cap);
            if (occ && occ.color !== color)
                res.push({ from, to: cap });
        }
